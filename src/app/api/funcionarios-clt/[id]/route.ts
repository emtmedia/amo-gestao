...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.funcionarioCLT.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.funcionarioCLT.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Colaborador CLT", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT funcionarioCLT error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.funcionarioCLT.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Colaborador CLT", params.id)
