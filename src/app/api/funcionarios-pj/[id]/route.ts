...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.funcionarioPJ.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const dateFields = ['dataConstituicao', 'dataContratacao']
    ...
    const item = await prisma.funcionarioPJ.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Colaborador PJ", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT funcionarioPJ error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.funcionarioPJ.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Colaborador PJ", params.id)
