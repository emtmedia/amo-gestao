...
export async function GET() {
  try {
    const items = await prisma.funcionarioPJ.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.funcionarioPJ.create({ data: body })
    await logAudit("CRIAR", "Colaborador PJ", item.id, `Criado: ${JSON.stringify(body).substring(0,200)}`)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('POST funcionarioPJ error:', error)
