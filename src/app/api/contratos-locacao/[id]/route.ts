...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.contratoLocacao.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const dateFields = ['dataInicioLocacao']
    ...
    const item = await prisma.contratoLocacao.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Contrato Locação", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT contratoLocacao error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.contratoLocacao.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Contrato Locação", params.id)
