...
export async function GET() {
  try {
    const items = await prisma.contratoLocacao.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.contratoLocacao.create({ data: body })
    await logAudit("CRIAR", "Contrato Locação", item.id, `Criado: ${JSON.stringify(body).substring(0,200)}`)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('POST contratoLocacao error:', error)
