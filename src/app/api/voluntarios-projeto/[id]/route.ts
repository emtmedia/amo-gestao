...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.voluntarioProjeto.findUnique({ where: { id: params.id } })
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.voluntarioProjeto.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Voluntário Projeto", params.id)
