...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.receitaPessoaFisica.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.receitaPessoaFisica.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Receita PF", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT receitaPessoaFisica error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.receitaPessoaFisica.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Receita PF", params.id)
