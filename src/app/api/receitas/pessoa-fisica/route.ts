...
export async function GET() {
  try {
    const items = await prisma.receitaPessoaFisica.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.receitaPessoaFisica.create({ data: body })
    await logAudit("CRIAR", "Receita PF", item.id, `Criado: ${JSON.stringify(body).substring(0,200)}`)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('POST /api/receitas/pessoa-fisica error:', error)
