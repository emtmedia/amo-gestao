...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.receitaEvento.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.receitaEvento.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Receita Evento", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT receitaEvento error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.receitaEvento.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Receita Evento", params.id)
