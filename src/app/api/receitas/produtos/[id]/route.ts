...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.receitaProduto.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.receitaProduto.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Receita Produto", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT receitaProduto error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.receitaProduto.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Receita Produto", params.id)
