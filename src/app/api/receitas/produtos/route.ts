...
export async function GET() {
  try {
    const items = await prisma.receitaProduto.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.receitaProduto.create({ data: body })
    await logAudit("CRIAR", "Receita Produto", item.id, `Criado: ${JSON.stringify(body).substring(0,200)}`)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('POST /api/receitas/produtos error:', error)
