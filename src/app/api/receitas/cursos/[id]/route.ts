...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.receitaCurso.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.receitaCurso.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Receita Curso", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT receitaCurso error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.receitaCurso.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Receita Curso", params.id)
