...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.outraReceita.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.outraReceita.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Outra Receita", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT outraReceita error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.outraReceita.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Outra Receita", params.id)
