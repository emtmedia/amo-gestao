...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.consolidacaoEvento.findUnique({ where: { id: params.id } })
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.consolidacaoEvento.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Consolidação Evento", params.id)
