...
export async function GET() {
  try {
    const items = await prisma.despesaConservacao.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.despesaConservacao.create({ data: body })
    await logAudit("CRIAR", "Despesa Conservação", item.id, `Criado: ${JSON.stringify(body).substring(0,200)}`)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('POST /api/despesas/conservacao error:', error)
