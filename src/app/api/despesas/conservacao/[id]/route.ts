...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.despesaConservacao.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.despesaConservacao.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Despesa Conservação", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT despesaConservacao error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.despesaConservacao.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Despesa Conservação", params.id)
