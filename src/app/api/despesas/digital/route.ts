...
export async function GET() {
  try {
    const items = await prisma.despesaDigital.findMany({ orderBy: { createdAt: 'desc' } })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const item = await prisma.despesaDigital.create({ data: body })
    await logAudit("CRIAR", "Despesa Digital", item.id, `Criado: ${JSON.stringify(body).substring(0,200)}`)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('POST /api/despesas/digital error:', error)
