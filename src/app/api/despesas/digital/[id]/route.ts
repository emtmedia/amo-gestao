...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.despesaDigital.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const item = await prisma.despesaDigital.update({ where: { id: params.id }, data: body })
    await logAudit("EDITAR", "Despesa Digital", params.id)
    return NextResponse.json({ success: true, data: item })
  } catch (error: unknown) {
    console.error('PUT despesaDigital error:', error)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    await prisma.despesaDigital.delete({ where: { id: params.id } })
    await logAudit("EXCLUIR", "Despesa Digital", params.id)
