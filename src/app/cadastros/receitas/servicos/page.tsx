...
const emptyForm = { servicoId:'', valorReceita:'', dataEntrada:'', contaBancariaId:'', metodoTransferenciaId:'', projetoDirecionado:'', eventoDirecionado:'', observacoes:'', arquivosReferencia: '' }
...
export default function Page() {
  ...
  const handleSave = async () => {
    if (!form.servicoId || !form.valorReceita || !form.dataEntrada || !form.contaBancariaId || !form.metodoTransferenciaId || !form.projetoDirecionado || !form.eventoDirecionado) { showToast('Preencha todos os campos obrigatórios','error'); return }
    setSaving(true)
    try {
      ...
      const res = await fetch(editing ? `/api/receitas/servicos/${editing.id}` : '/api/receitas/servicos', { method: editing?'PUT':'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) })
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-3"><Wrench className="w-7 h-7 text-navy-700" /><div><h1 className="page-title">Receita de Serviços</h1><p className="text-sm text-navy-400">{data.length} registros</p></div></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} alert={modalAlert} title={editing?'Editar':'Nova Receita de Serviços'} size="xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="form-group md:col-span-2"><label>Serviço<span className="required-star">*</span></label><select value={form.servicoId} onChange={e=>setForm(p=>({...p,servicoId:e.target.value}))} className="form-input"><option value="">Selecione...</option>{opcoes.map(o=><option key={o.id} value={o.id}>{o.nome}</option>)}</select>
              {opcoes.length === 0 && (
                <p className="text-xs text-amber-600 mt-1.5 flex items-center gap-1">
                  🔧 Nenhum(a) serviço cadastrado(a) ainda.{' '}
                  <a href="/cadastros/auxiliares?tab=servicos" className="underline font-medium hover:text-amber-800" target="_blank" rel="noreferrer">
                    Cadastrar agora →
                  </a>
                </p>
              )}
