...
interface Rec { id: string; valorContribuicao: number; dataEntrada: string; [key: string]: unknown }
...
const emptyForm = { nomeEmpresa:"",telefoneContato:"",telefoneFixo:"",emailContato:"", dataEntrada:'', contaBancariaId:'', valorContribuicao:'', metodoTransferenciaId:'', projetoDirecionado:'', eventoDirecionado:'', observacoes:'', arquivosReferencia: '' }
...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const [rd, rc, rm, rp, re] = await Promise.all([fetch('/api/receitas/pessoa-juridica'), fetch('/api/contas-bancarias'), fetch('/api/auxiliares?tipo=metodos'), fetch('/api/projetos'), fetch('/api/eventos')])
      const [jd, jc, jm, jp, je] = await Promise.all([rd.json(), rc.json(), rm.json(), rp.json(), re.json()])
      if (jd.success) setData(jd.data); if (jc.success) setContas(jc.data); if (jm.success) setMetodos(jm.data); if (jp.success) setProjetos(jp.data); if (je.success) setEventos(je.data)
  ...
  const handleSave = async () => {
    if (!form.valorContribuicao || !form.dataEntrada || !form.contaBancariaId || !form.metodoTransferenciaId || !form.projetoDirecionado || !form.eventoDirecionado) { showToast('Preencha todos os campos obrigatórios','error'); return }
    setSaving(true)
    try {
      const payload = { ...form, valorContribuicao: parseBRL(form.valorContribuicao), observacoes: form.observacoes||null, arquivosReferencia: form.arquivosReferencia||null }
      const res = await fetch(editing ? `/api/receitas/pessoa-juridica/${editing.id}` : '/api/receitas/pessoa-juridica', { method: editing ? 'PUT' : 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) })
  ...
  const handleDelete = async (id: string) => {
    try { const r = await fetch(`/api/receitas/pessoa-juridica/${id}`,{method:'DELETE'}); const j = await r.json(); if (j.success) { showToast('Removido!'); setDeleteConfirm(null); fetchData() } } catch { showToast('Erro','error') }
  }
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-3"><Building2 className="w-7 h-7 text-navy-700" /><div><h1 className="page-title">Receita Pessoa Jurídica</h1><p className="text-sm text-navy-400">{data.length} registros</p></div></div>
        <button onClick={openCreate} className="btn-primary"><Plus className="w-4 h-4" /> Nova Receita</button>
      </div>
      <div className="card">{loading ? <div className="text-center py-8 text-navy-400">Carregando...</div> : <DataTable data={data} columns={[{key:'nomeEmpresa',label:'Empresa'},{key:'telefoneContato',label:'Telefone'},{key:'emailContato',label:'E-mail',render:v=>v?String(v):<span className="text-navy-300">—</span>},{key:'valorContribuicao',label:'Valor',render:(v)=>fmtMoney(Number(v))},{key:'dataEntrada',label:'Data Entrada',render:(v)=>fmtDate(String(v))},{key:'projetoDirecionado',label:'Projeto',render:v=>v?String(v).substring(0,20):<span className="text-navy-300">—</span>},{key:'eventoDirecionado',label:'Evento',render:v=>v?String(v).substring(0,20):<span className="text-navy-300">—</span>}]} onEdit={openEdit} onDelete={(row)=>setDeleteConfirm(String(row.id))} searchKeys={[]} />}</div>
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} alert={modalAlert} title={editing?'Editar':'Nova Receita Pessoa Jurídica'} size="xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          
          {inp("nomeEmpresa","Nome da Empresa Contribuinte",true)}
          <PhoneInput label="Telefone Celular de Contato" required value={form.telefoneContato} onChange={v=>setForm(p=>({...p,telefoneContato:v}))}/>
          <PhoneInput label="Telefone Fixo de Contato" value={form.telefoneFixo} onChange={v=>setForm(p=>({...p,telefoneFixo:v}))}/>
          {inp("emailContato","E-mail do Contato",false,"email")}
          <DateInput label="Data da Entrada da Contribuição" required value={form.dataEntrada} onChange={v=>setForm(p=>({...p,dataEntrada:v}))}/>
