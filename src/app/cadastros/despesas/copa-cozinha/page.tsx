...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [rd, rf, rt, rcb, rm, rp, re] = await Promise.all([fetch('/api/despesas/copa-cozinha'), fetch('/api/fornecedores'), fetch('/api/auxiliares?tipo=itensCopa'), fetch('/api/contas-bancarias'), fetch('/api/auxiliares?tipo=metodos'), fetch('/api/projetos'), fetch('/api/eventos')])
  ...
  const handleDelete = async (id: string) => {
    try { const r = await fetch(`/api/despesas/copa-cozinha/${id}`,{method:'DELETE'}); const j = await r.json(); if (j.success) { showToast('Removido!'); setDeleteConfirm(null); fetchData() } } catch { showToast('Erro','error') }
  }

  const fmtNomeForn = (v: unknown) => { const n = fornecedores.find(f => f.id === String(v)); return n ? n.nome : String(v) }
  const fmtItemCopa = (v: unknown) => { const n = tiposServico.find(t => t.id === String(v)); return n ? n.nome : String(v) }
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-3"><Coffee className="w-7 h-7 text-navy-700" /><div><h1 className="page-title">Copa e Cozinha</h1><p className="text-sm text-navy-400">{data.length} registros</p></div></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} alert={modalAlert} title={editing?'Editar':'Nova Copa e Cozinha'} size="xl">
