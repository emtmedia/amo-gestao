...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [rd, rf, rt, rcb, rm, rp, re] = await Promise.all([fetch('/api/despesas/locacao'), fetch('/api/fornecedores'), fetch('/api/auxiliares?tipo=itensAlugados'), fetch('/api/contas-bancarias'), fetch('/api/auxiliares?tipo=metodos'), fetch('/api/projetos'), fetch('/api/eventos')])
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-3"><Package className="w-7 h-7 text-navy-700" /><div><h1 className="page-title">Locação de Serviços & Equipamentos</h1><p className="text-sm text-navy-400">{data.length} registros</p></div></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} alert={modalAlert} title={editing?'Editar':'Nova Locação de Serviços & Equipamentos'} size="xl">
