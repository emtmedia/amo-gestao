...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [rd, rf, rt, rcb, rm, rp, re] = await Promise.all([fetch('/api/despesas/consumo'), fetch('/api/fornecedores'), fetch('/api/auxiliares?tipo=tiposConsumo'), fetch('/api/contas-bancarias'), fetch('/api/auxiliares?tipo=metodos'), fetch('/api/projetos'), fetch('/api/eventos')])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(editing ? `/api/despesas/consumo/${editing.id}` : '/api/despesas/consumo', { method: editing?'PUT':'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) })
  ...
  const handleDelete = async (id: string) => {
    try { const r = await fetch(`/api/despesas/consumo/${id}`,{method:'DELETE'}); const j = await r.json(); if (j.success) { showToast('Removido!'); setDeleteConfirm(null); fetchData() } } catch { showToast('Erro','error') }
  }

  const fmtNomeForn = (v: unknown) => { const n = fornecedores.find(f => f.id === String(v)); return n ? n.nome : String(v) }
  const fmtTipoConsumo = (v: unknown) => { const n = tiposServico.find(t => t.id === String(v)); return n ? n.nome : String(v) }
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-3"><Zap className="w-7 h-7 text-navy-700" /><div><h1 className="page-title">Contas de Consumo</h1><p className="text-sm text-navy-400">{data.length} registros</p></div></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} alert={modalAlert} title={editing?'Editar':'Nova Contas de Consumo'} size="xl">
