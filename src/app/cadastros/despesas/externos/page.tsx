...
interface Desp { id: string; valor: number; dataPagamento: string; [key: string]: unknown }
...
const emptyForm = { fornecedorId:'', servicoPrestadoId:'', valor:'', dataPagamento:'', contaBancariaId:'', metodoTransferenciaId:'', projetoDirecionado:'', eventoDirecionado:'', observacoes:'', arquivosReferencia: '' }
...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [rd, rf, rt, rcb, rm, rp, re] = await Promise.all([fetch('/api/despesas/externos'), fetch('/api/fornecedores'), fetch('/api/auxiliares?tipo=servicosPrestados'), fetch('/api/contas-bancarias'), fetch('/api/auxiliares?tipo=metodos'), fetch('/api/projetos'), fetch('/api/eventos')])
      const [jd, jf, jt, jcb, jm, jp, je] = await Promise.all([rd.json(), rf.json(), rt.json(), rcb.json(), rm.json(), rp.json(), re.json()])
      if (jd.success) setData(jd.data); if (jf.success) setFornecedores(jf.data); if (jt.success) setTiposServico(jt.data); if (jcb.success) setContas(jcb.data); if (jm.success) setMetodos(jm.data); if (jp.success) setProjetos(jp.data); if (je.success) setEventos(je.data)
  ...
  const handleSave = async () => {
    if (!form.fornecedorId || !form.servicoPrestadoId || !form.valor || !form.dataPagamento || !form.metodoTransferenciaId) { showToast('Preencha todos os campos obrigatórios','error'); return }
    setSaving(true)
    try {
      const payload = { ...form, valor: parseBRL(form.valor), contaBancariaId: form.contaBancariaId||null, observacoes: form.observacoes||null, arquivosReferencia: form.arquivosReferencia||null, projetoDirecionado: form.projetoDirecionado||null, eventoDirecionado: form.eventoDirecionado||null }
      const res = await fetch(editing ? `/api/despesas/externos/${editing.id}` : '/api/despesas/externos', { method: editing?'PUT':'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) })
  ...
  const handleDelete = async (id: string) => {
    try { const r = await fetch(`/api/despesas/externos/${id}`,{method:'DELETE'}); const j = await r.json(); if (j.success) { showToast('Removido!'); setDeleteConfirm(null); fetchData() } } catch { showToast('Erro','error') }
  }

  const fmtNomeForn = (v: unknown) => { const n = fornecedores.find(f => f.id === String(v)); return n ? n.nome : String(v) }
  const fmtServico = (v: unknown) => { const n = tiposServico.find(t => t.id === String(v)); return n ? n.nome : String(v) }
  ...
  const cols = [{key:'fornecedorId',label:'Fornecedor',render:fmtNomeForn},{key:'servicoPrestadoId',label:'Serviço Prestado',render:fmtServico},{key:'valor',label:'Valor Pgto',render:(v: unknown)=>fmtMoney(Number(v))},{key:'dataPagamento',label:'Pagamento',render:(v: unknown)=>fmtDate(String(v))},{key:'metodoTransferenciaId',label:'Método Pgto',render:(v: unknown)=>metodos.find(m=>m.id===String(v))?.nome||'-'},{key:'projetoDirecionado',label:'Projeto',render:(v: unknown)=>{ const p=projetos.find(x=>x.id===String(v)); return p?<span>{p.nome}</span>:<span className="text-navy-300">—</span>}},{key:'eventoDirecionado',label:'Evento',render:(v: unknown)=>{ const e=eventos.find(x=>x.id===String(v)); return e?<span>{e.nome}</span>:<span className="text-navy-300">—</span>}}]
  return (
    <div>
      ...
      <div className="page-header">
        <div className="flex items-center gap-3"><Hammer className="w-7 h-7 text-navy-700" /><div><h1 className="page-title">Serviços Externos</h1><p className="text-sm text-navy-400">{data.length} registros</p></div></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} alert={modalAlert} title={editing?'Editar':'Nova Serviços Externos'} size="xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="form-group md:col-span-2"><label>Serviço Prestado<span className="required-star">*</span></label><select value={form.servicoPrestadoId} onChange={e=>setForm(p=>({...p,servicoPrestadoId:e.target.value}))} className="form-input"><option value="">Selecione...</option>{tiposServico.map(t=><option key={t.id} value={t.id}>{t.nome}</option>)}</select></div>
          <CurrencyInput label="Valor do Serviço (R$)" required value={form.valor} onChange={v=>setForm(p=>({...p,valor:v}))}/>
